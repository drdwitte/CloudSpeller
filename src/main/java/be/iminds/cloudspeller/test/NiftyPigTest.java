package be.iminds.cloudspeller.test;
...
public class NiftyPigTest {
    ...
    private ThreadLocal<PigServer> pig = new ThreadLocal<PigServer>();
    private ThreadLocal<Cluster> cluster = new ThreadLocal<Cluster>();
     ...
     */
    public Cluster getCluster() throws ExecException {
        if (cluster.get() == null) {
    ...
    /**
     * Resets the cluster between processing iterations
     * @throws ExecException
     */
    private void resetCluster() throws ExecException {
        pig.set(new PigServer(ExecType.LOCAL));
        cluster.set(new Cluster(pig.get().getPigContext()));
    }
    ...
    public DataSetReport validate(DataSetValidator.Builder validatorBuilder) throws IOException, ParseException {
        execute();
